...
const RequestAdminAccess = ({ userId }: RequestAdminAccessProps) => {
    ...
    if (existingRequest === undefined) {
        return (
            <div className="flex items-center justify-center min-h-[70vh]">
                <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-green-1" />
            </div>
        );
    }
    ...
    if (existingRequest) {
        return (
            <div className="bg-green-1/10 border border-green-1/20 rounded-xl p-6 text-center">
                <div className="flex justify-center mb-4">
                    <Shield className="w-12 h-12 text-green-1" />
                </div>
        ...
        <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
            <div className="bg-black-1/30 border border-gray-800 rounded-xl p-8 max-w-md w-full">
                <div className="flex justify-center mb-6">
                    <div className="bg-green-1/20 p-4 rounded-full">
                        <Shield className="w-8 h-8 text-green-1" />
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <textarea
                            ...
                            className="w-full bg-black-2/50 border border-gray-800 rounded-lg p-3 text-white-1 placeholder:text-white-3 focus:outline-none focus:border-green-1 transition-colors min-h-[120px]"
                    ...
                    <button
                        ...
                        className="w-full bg-green-1 hover:bg-green-2 disabled:opacity-50 disabled:hover:bg-green-1 text-white font-medium rounded-lg py-3 px-4 transition-colors flex items-center justify-center gap-2"
                    >
                        {isSubmitting ? (
                            "Submitting..."
                        ) : (
